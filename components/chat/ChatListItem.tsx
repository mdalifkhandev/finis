  ...
  return (
    ...
    >
      <View className="flex-row items-center">
        <Image source={{ uri: item.avatarUrl }} className="h-[56px] w-[56px] rounded-full" />

        <View className="ml-4 flex-1">
          <Text className="text-[16px] font-semibold text-[#2B2B2B]">{item.name}</Text>
        ...
        <View className="items-end">
          ...
          {item.unreadCount > 0 ? (
            <View className="mt-2.5 h-[22px] w-[22px] items-center justify-center rounded-full bg-[#1D5478]">
              <Text className="text-[13px] font-medium text-white">{item.unreadCount}</Text>
            </View>
          ) : null}
        </View>
      </View>
