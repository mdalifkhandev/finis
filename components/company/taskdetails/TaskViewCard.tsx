    ...
    return (
        <View className="mx-5 my-6 rounded-[24px] bg-white p-6 shadow-sm border border-slate-50">
            <View className="items-center pb-5">
                <Text className="text-[22px] font-bold text-[#1F2937]">{workerName}</Text>
            ...
            <View className="space-y-4">
                <Text className="text-[15px] font-semibold text-[#374151] mb-2">{title}</Text>
                ...
                <View className="flex-row justify-between mb-3">
                    ...
                    <Text className="text-[14px] font-medium text-[#111827]">{roomNo}</Text>
                </View>

                <View className="flex-row justify-between mb-3">
                    <Text className="text-[14px] text-slate-500">Start Time:</Text>
                    <Text className="text-[14px] font-medium text-[#111827]">{startTime}</Text>
                </View>

                <View className="flex-row justify-between mb-3">
                    <Text className="text-[14px] text-slate-500">End Time:</Text>
                    <Text className="text-[14px] font-medium text-[#111827]">{endTime}</Text>
                </View>
            ...
            <View className="rounded-[16px] bg-[#F7F9FB] p-5 mb-6">
                <Text className="text-[14px] font-bold text-[#1F2937] mb-2">Description</Text>
            ...
            >
                ...
                >
                    <Pressable
                        ...
                        style={{ paddingBottom: Math.max(insets.bottom, 20) + 12, borderTopEndRadius: 30, borderTopLeftRadius: 30 }}
                    >
                        <View className="items-center mb-6">
                            <View className="h-1.5 w-16 rounded-full bg-[#E5E7EB]" />
                        </View>

                        <Text className="text-[24px] font-bold text-[#0D1B34] mb-4">Before You Start</Text>
                        <Text className="text-[16px] leading-[22px] text-[#2D455F] mb-6">
                            Please take a photo of the current condition before starting the task. This photo is required for verification.
                        </Text>
                        ...
                        {!takenPhoto ? (
                            ...
                            >
                                <Text className="text-[18px] font-semibold text-[#1F2937]">Take photo</Text>
                            </TouchableOpacity>
                        ) : (
                            <View className="rounded-[24px] border border-[#E5E7EB] p-3">
                                ...
                                <View className="flex-row gap-3">
                                    ...
                                    >
                                        <Text className="text-[16px] font-semibold text-[#1F2937]">Retake</Text>
                                    </TouchableOpacity>
                                    <TouchableOpacity
                                        activeOpacity={0.8}
                                        ...
                                        style={{ backgroundColor: '#CCECFF' }}
                                    >
                                        <Text className="text-[16px] font-semibold text-[#1F2937]">Confirm</Text>
                                    </TouchableOpacity>
                                </View>
                            </View>
                        )}
